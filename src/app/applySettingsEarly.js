(() => {
    ...
    const applyCssVars = (settings) => {
        ...
        const uiFontSize = Number(settings?.ui?.fontSize ?? DEFAULTS.ui.fontSize) || 13;
        root.style.setProperty('--font-ui', uiFontFamily === 'Inter' ? 'var(--font-ui)' : uiFontFamily);
        root.style.setProperty('--font-size-ui', `${uiFontSize}px`);

        // Legacy UI scale support (deprecated, use ui.fontSize instead)
        const uiScale = Number(settings?.fonts?.ui?.scalePercent ?? 100) || 100;
        if (uiScale !== 100 && !settings?.ui?.fontSize) {
            const uiSizeBase = 13;
        ...
        root.style.setProperty('--font-code', editorFamily);
        ...
        root.style.setProperty('--font-terminal', terminalFamily);
