(() => {
    ...
    const openContextMenu = ({ controller, x, y, items, ctx, onAction, rootMeta } = {}) => {
        const menu = window.AhmadIDEModules?.ui?.menu;
        const ctrl = controller || menu?.controller || menu?.createMenuController?.({});
        if (!ctrl || typeof ctrl.openAtPoint !== 'function') return null;
        return ctrl.openAtPoint({ x, y, items, ctx, onAction, rootMeta });
    };
    ...
    if (typeof window !== 'undefined') {
        ...
        window.AhmadIDEModules.ui.menu.openContextMenu = openContextMenu;
    }
})();
